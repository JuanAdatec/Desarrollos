...
FROM   OPENQUERY ([SRVERP\SIESA],'
	SELECT     
		...
		CAST(RTRIM(ISNULL(TCC.CLIENTE_TIPO, ''SIN CLASIFICAR'')) AS VARCHAR(30)) AS CLIENTE_TIPO, 
		...
		CAST(RTRIM(CN.F015_DIRECCION1) + '' '' + RTRIM(ISNULL(CN.f015_id_barrio, '''')) AS VARCHAR(300)) AS CLIENTE_DIRECCION, 
		...
		f015_id_ciudad
FROM        
	[UnoEE_Real].[dbo].T201_MM_CLIENTES C WITH(NOLOCK)  INNER JOIN [UnoEE_Real].[dbo].T200_MM_TERCEROS T WITH(NOLOCK)  ON 
		C.F201_ROWID_TERCERO = T .F200_ROWID 
		AND C.F201_ID_CIA = T .F200_ID_CIA 
	INNER JOIN [UnoEE_Real].[dbo].T015_MM_CONTACTOS CN WITH(NOLOCK)  ON 
		C.F201_ROWID_CONTACTO = CN.F015_ROWID 
		AND C.F201_ID_CIA = CN.F015_ID_CIA 
	INNER JOIN [UnoEE_Real].[dbo].T278_CO_TIPO_CLI TC WITH(NOLOCK)  ON 
		convert(varchar, TC.F278_ID) = convert(varchar, C.F201_ID_TIPO_CLI )
		AND TC.F278_ID_CIA = C.F201_ID_CIA 
	INNER JOIN [UnoEE_Real].[dbo].T013_MM_CIUDADES CY WITH(NOLOCK)  ON 
		CN.F015_ID_PAIS = CY.F013_ID_PAIS 
		AND CN.F015_ID_DEPTO = CY.F013_ID_DEPTO 
		AND CN.F015_ID_CIUDAD = CY.F013_ID 
	left JOIN [UnoEE_Real].[dbo].T012_MM_DEPTOS D WITH(NOLOCK)  ON 
		CN.F015_ID_PAIS = D .F012_ID_PAIS 
		AND CN.F015_ID_DEPTO = D .F012_ID 
	inner JOIN [UnoEE_Real].[dbo].T208_MM_CONDICIONES_PAGO CP WITH(NOLOCK)  ON 
		C.F201_ID_CIA = CP.F208_ID_CIA 
		AND C.F201_ID_COND_PAGO = CP.F208_ID 
	INNER JOIN [UnoEE_Real].[dbo].T010_MM_COMPANIAS CM WITH(NOLOCK)  ON 
		C.F201_ID_CIA = CM.F010_ID 
	LEFT JOIN  [UnoEE_Real].[dbo].[t047_mm_cliente_base_retencion] CBR WITH(NOLOCK)  ON
	...
	LEFT JOIN [UnoEE_Real].[dbo].[t040_mm_llaves_retencion] LLR WITH(NOLOCK)  ON 
	...
	LEFT JOIN (SELECT 
				f207_rowid_tercero, 
				f207_id_sucursal,
				RTRIM(ISNULL(ISNULL([B2B], [B2C]), '''')) AS CLIENTE_TIPO
			FROM   
			(
				SELECT 
					f207_rowid_tercero,
					...
					f206_descripcion
				FROM 
					[UnoEE_Real].[dbo].t207_mm_criterios_clientes 
					INNER JOIN [UnoEE_Real].[dbo].t206_mm_criterios_mayores ON
						f207_id_cia = f206_id_cia
						AND f207_id_plan_criterios = f206_id_plan
						AND f207_id_criterio_mayor = f206_id
				WHERE 
					f207_id_cia = 1 
					AND f207_id_plan_criterios IN (''B2B'', ''B2C'' )
			) AS p
			PIVOT  
			(  
				MAX (f206_descripcion) FOR f207_id_plan_criterios IN  ( [B2B], [B2C] )  
			) AS pvt) AS TCC ON 
		TCC.f207_rowid_tercero = C.F201_ROWID_TERCERO
		AND TCC.f207_id_sucursal = C.F201_ID_SUCURSAL
WHERE     
             c.F201_ID_CIA = 1 
